...
public class LabController {
    ...
    @Log(value = "以音搜图")
    @PostMapping(value = "/audioRetrieve")
    @ApiOperation("以音搜图")
    public CommonResult<PageResult<JSONObject>> audioRetrieve(AudioRetrieveParam audioRetrieveParam) throws IOException { ;
        if (audioRetrieveParam.getFile().isEmpty())
            return new CommonResult<PageResult<JSONObject>>().data(null).fail().message("请选择上传文件");
        PageResult<JSONObject> pageResult = this.labService.retrieveImageByAudio(audioRetrieveParam);
        return new CommonResult<PageResult<JSONObject>>().success().data(pageResult).message("以音搜图完成");
    }
